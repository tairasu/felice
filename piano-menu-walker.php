...
class Mobile_Piano_Menu_Walker extends Walker_Nav_Menu {
    ...
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0) {
        ...
        if ($depth === 0) {
            ...
            $output .= sprintf('<li class="pl-4 bg-white/75 border border-orange-900/20 rounded-lg h-16 w-36 shadow-lg p-2 hover:bg-orange-100 hover:scale-95 transition duration-200 ease-in-out"><a href="%s" class="flex font-bold h-full w-full items-center justify-start break-keep text-center">%s</a></li>',
                esc_url($item->url),
                esc_html($item->title)
            );
